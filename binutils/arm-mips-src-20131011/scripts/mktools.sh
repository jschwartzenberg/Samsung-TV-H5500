...
njobs () {
	echo $((`grep -c ^processor /proc/cpuinfo` * 3 / 2))
}

SCRIPT_RUN_DIR=$(cd "$(dirname $0)"; pwd)
SCRIPT_BASE_NAME=`basename $0`
pushd ${SCRIPT_RUN_DIR}
popd
...
help(){
  ...
  echo "`ls ${SCRIPT_RUN_DIR}/toolchain.cfg/*.config | sed -e "s/^.*\///" | sed -e "s/^/    /"`"
  ...
  echo "Use -m to prepare a native instance of compiler"
  echo "Use -j <number> to specify -j option to make (default is 1.5 * number-of-cores)"
  echo "Use -h print this help"
}

while getopts "c:s:b:mi:hnj:" OPTION
do
  case $OPTION in
    c)
      CONFIG_FILE=${SCRIPT_RUN_DIR}/toolchain.cfg/"$OPTARG"
      ;;
    n)
      NON_STANDARD=true
      ;;
...
GCC_SPECS='--with-specs=%{funwind-tables|fno-unwind-tables|mabi=*|ffreestanding|nostdlib:;:-funwind-tables} %{O3:%{!funswitch-loops:%{!fno-unswitch-loops:-fno-unswitch-loops}}}'
GCC_SPECS_FINAL='--with-specs=%{!fipa-sra:%{!fno-ipa-sra:-fno-ipa-sra}} %{!gstrict-dwarf:%{!gno-strict-dwarf:-gstrict-dwarf}} %{funwind-tables|fno-unwind-tables|mabi=*|ffreestanding|nostdlib:;:-funwind-tables} %{O3:%{!funswitch-loops:%{!fno-unswitch-loops:-fno-unswitch-loops}}}'
...
#EGLIBC definitions
EGLIBC_NAME=glibc
EGLIBC_ADD_ONS_LIST="ports,nptl"
...
# common function to build native tools
# this function do configure, make, make install.
# if you need to do some additional action, you should write your own wrapper.                                                                                 
# arguments:
#           $1 - source dir of tool
#           $2 - list of option to ./configure script                                                                                                          
#           $4 - file to output log
...
mk_native_tool () {
   ...
   TOOL_SRC_DIR=`basename $1`
   TOOL_BUILD_DIR="$OBJS_BUILD_DIR/$TOOL_SRC_DIR-native"
   ...
   PATH=${TOOLS_DIR}:${PATH} \
   LD_LIBRARY_PATH=$PREFIX_CROSS/lib:${LD_LIBRARY_PATH} \
   CFLAGS="-O2 -g" \
   CXXFLAGS="-O2 -g" \
...
echo_start "${BU_NAME}_native"

   prepare_to_build ${BU_NAME} "${BU_NAME}_native" "${BU_NAME}_native.log"

    LDFLAGS_BU_BEFORE=$LDFLAGS
    CPPFLAGS_BU_BEFORE=$CPPFLAGS

    LDFLAGS="-L${PREFIX_CROSS}/lib"
    CPPFLAGS="-I${PREFIX_CROSS}/include"

    make_clean_dir "$OBJS_BUILD_DIR/${BU_NAME}_native" "${LOG_DIR}/${BU_NAME}_native.log" 2>&1

    pushd "$OBJS_BUILD_DIR/${BU_NAME}_native"

    echo "Configure native ${BU_NAME}"
    echo "Configure native ${BU_NAME}" >> "${LOG_DIR}/${BU_NAME}_native.log" 2>&1
    ...
    ${SRC_BUILD_DIR}/${BU_NAME}/configure \
      ...
      --with-bugurl=${WITH_BUGURL} \
      --disable-nls \
      --enable-poison-system-directories --enable-install-libiberty --enable-install-libbfd --enable-shared \
      --disable-werror  >> "${LOG_DIR}/${BU_NAME}_native.log" 2>&1
      check_success

    echo "Make native ${BU_NAME}"
    echo "Make native ${BU_NAME}" >> "${LOG_DIR}/${BU_NAME}_native.log" 2>&1
    ...
    AR=${CROSS_TOOLS_PREFIX}-ar \
    RANLIB=${CROSS_TOOLS_PREFIX}-ranlib \
         make ${MK_FLAGS} >> "${LOG_DIR}/${BU_NAME}_native.log" 2>&1


    echo "Install native ${BU_NAME}"
    echo "Install native ${BU_NAME}" >> "${LOG_DIR}/${BU_NAME}_native.log" 2>&1
    ...
    DESTDIR=${SYSROOT} \
    DESTDIR_PATH=${SYSROOT} \
    make install >> "${LOG_DIR}/${BU_NAME}_native.log" 2>&1
    check_success

    CPPFLAGS=$CPPFLAGS_BU_BEFORE
    LDFLAGS=$LDFLAGS_BU_BEFORE
...
echo_start ${GMP_NAME}_native

prepare_to_build ${GMP_NAME} ${GMP_NAME}_native "${GMP_NAME}_native.log"

    CPPFLAGS=-fexceptions
    mk_native_tool "${SRC_BUILD_DIR}/${GMP_NAME}" "--prefix=/usr \
                          --build=$BUILD \
                          --host=$TARGET \
                          --target=$TARGET \
                          --enable-cxx \
                          --enable-shared \
                          --enable-static" "${LOG_DIR}/${GMP_NAME}_native.log"
    CPPFLAGS=""

#                          --build=$BUILD \
#                          --host=$HOST \
echo_stop ${GMP_NAME}_native
}
...
echo_start ${MPFR_NAME}_native

prepare_to_build ${MPFR_NAME} ${MPFR_NAME}_native "${MPFR_NAME}_native.log"

    mk_native_tool "${SRC_BUILD_DIR}/${MPFR_NAME}" "--prefix=/usr \
                       --build=$BUILD \ 
                       --host=$TARGET \
                       --target=$TARGET \
                       --disable-shared \
                       --disable-nls \
                       --with-gmp-include=${SYSROOT}/usr/include \
                       --enable-static" "${LOG_DIR}/${MPFR_NAME}_native.log"

echo_stop ${MPFR_NAME}_native
}
...
mk_native_mpc(){
echo_start ${MPC_NAME}_native

prepare_to_build ${MPC_NAME} ${MPC_NAME}_native "${MPC_NAME}_native.log"

    mk_native_tool "${SRC_BUILD_DIR}/${MPC_NAME}" "--prefix=/usr \
                      --build=$BUILD \
                      --host=$TARGET \
                      --target=$TARGET \
                      --with-gmp-include=${SYSROOT}/usr/include \
                      --with-mpfr-include=${SYSROOT}/usr/include \
                      --disable-nls --disable-shared --enable-static" "${LOG_DIR}/${MPC_NAME}_native.log"

echo_stop ${MPC_NAME}_native
}
...
mk_native_ppl(){
echo_start ${PPL_NAME}_native

prepare_to_build ${PPL_NAME} ${PPL_NAME}_native "${PPL_NAME}_native.log"

    mk_native_tool "${SRC_BUILD_DIR}/${PPL_NAME}" "--prefix=/usr \
                      --build=$BUILD \
                      --host=$TARGET \
                      --target=$TARGET \
                      --disable-shared \
                      ...
                      LDFLAGS=-L${SYSROOT}/usr/lib " "${LOG_DIR}/${PPL_NAME}_native.log"

echo_stop ${PPL_NAME}_native
}
...
mk_native_cloog(){
echo_start ${CLOOG_NAME}_native

prepare_to_build ${CLOOG_NAME} ${CLOOG_NAME}_native "${CLOOG_NAME}_native.log"

        LDFLAGS_BEFORE=$LDFLAGS

LDFLAGS="-lstdc++ -lm -L${SYSROOT}/usr/lib"

    mk_native_tool "${SRC_BUILD_DIR}/${CLOOG_NAME}"  "--prefix=/usr \
                     --build=$BUILD \
                     --host=$TARGET \
                     --with-ppl=${SYSROOT}/usr \
                     --with-gmp=${SYSROOT}/usr \
                     --with-bits=gmp \
                     --disable-shared \
                     --enable-static \
                      --disable-nls \
                      CPPFLAGS=-I${SYSROOT}/usr/include " "${LOG_DIR}/${CLOOG_NAME}_native.log"

    LDFLAGS="$LDFLAGS_BEFORE"

echo_stop ${CLOOG_NAME}_native
}
...
echo_start "prepare_${EGLIBC_NAME}"
	...
	if [[ "$PRODUCT_VERSION" == "v3" ]]; then
		# move port folder if toolchain product version is equal to 3
...
echo_start "eglibc_0_headers"
    ...
    echo "Configure EGLIBC_0 headers " > ${EGLIBC_0_HEAD_LOG}
	    ...
	    ${GLIBC_TARGET_SPECIFIC_FLAGS} \
            ...
            --enable-hacker-mode \
            --enable-add-ons=${EGLIBC_ADD_ONS_LIST} \
...
mk_gcc_native(){
echo_start "${GCC_NAME}_native"

        GCC_NATIVE_BUILD_DIR="$OBJS_BUILD_DIR/${GCC_NAME}_native"
        GCC_NATIVE_LOG="${LOG_DIR}/${GCC_NAME}_native.log"

        echo "Build GCC native"
        echo "Build GCC native" > ${GCC_NATIVE_LOG}

        make_clean_dir $GCC_NATIVE_BUILD_DIR ${GCC_NATIVE_LOG}

        pushd $GCC_NATIVE_BUILD_DIR

     echo "Configure GCC native"
     echo "Configure GCC native" >> ${GCC_NATIVE_LOG}
            ...
            --with-gnu-as \
            --with-gnu-ld \
            --with-host-libstdcxx='-static-libgcc -Wl,-Bstatic,-lstdc++,-Bdynamic -lm' >> ${GCC_NATIVE_LOG} 2>&1
            check_success

        echo "Make GCC native"
        echo "Make GCC native" >> ${GCC_NATIVE_LOG}
        ...
        AR=${CROSS_TOOLS_PREFIX}-ar \
        RANLIB=${CROSS_TOOLS_PREFIX}-ranlib \
        make ${MK_FLAGS} build_tooldir=${TOOLS_DIR} >> ${GCC_NATIVE_LOG} 2>&1
        check_success

        echo "Install GCC native"
        echo "Install GCC native" >> ${GCC_NATIVE_LOG}
        ...
        DESTDIR=${SYSROOT} \
        DESTDIR_PATH=${SYSROOT} \
        PATH=${TOOLS_DIR}:$PATH LD_LIBRARY_PATH=$PREFIX_CROSS/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH} make ${MK_FLAGS} install >> ${GCC_NATIVE_LOG} 2>&1
        check_success

        cd $TOP_DIR

echo_stop "${GCC_NAME}_native"
}
...
LIBS_FINALIZE_LOG="$LOG_DIR/libs_finalize.log"
	...
	if [[ "$PRODUCT_VERSION" == "v3" ]]; then
		...
		# "$PRODUCT_VERSION" == "v4"
		cp -fv ${PREFIX_CROSS}/${TARGET}/lib/libstdc++.so.6.0.17 ${SYSROOT_DIR}/usr/lib/ >> ${LIBS_FINALIZE_LOG} 2>&1
	fi
 	check_success
