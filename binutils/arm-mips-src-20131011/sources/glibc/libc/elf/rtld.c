  ...
  if (main_map->l_info[ADDRIDX (DT_GNU_LIBLIST)]
      ...
      for (; r_list < r_listend && liblist < liblistend; r_list++)
	{
	  ...
	  if (! _dl_name_match_p (strtab + liblist->l_name, l))
		...
		_dl_debug_printf ("\n[vdprelink failed] %s: expect %s, found %s "
				  "in dependency order\n",
				  main_map->l_name[0] ? main_map->l_name : rtld_progname,
				  strtab + liblist->l_name, l->l_name);
	      prelink_brk = true;
	      break;
	    }
	  ...
	  if (l->l_addr)
	    {
	      if (__builtin_expect (GLRO_dl_debug_mask & DL_DEBUG_LIBS, 0))
		_dl_debug_printf ("\n[vdprelink failed] %s: %s load address error, "
				  "base: 0x%0x\n",
				  main_map->l_name[0] ? main_map->l_name : rtld_progname,
				  l->l_name, (unsigned int) l->l_addr);
	      prelink_brk = true;
	      break;
	    }
	  ...
	  if (l->l_info [VALIDX(DT_CHECKSUM)] == NULL
	    ...
	    {
	      if (__builtin_expect (GLRO_dl_debug_mask & DL_DEBUG_LIBS, 0))
		_dl_debug_printf ("\n[vdprelink failed] %s: %s expect 0x%0x, found "
				  "0x%0x checksum\n",
				  main_map->l_name[0] ? main_map->l_name : rtld_progname,
				  l->l_name, liblist->l_checksum,
				  l->l_info[VALIDX(DT_CHECKSUM)] ?
				  l->l_info[VALIDX(DT_CHECKSUM)]->d_un.d_val : 0);
	      prelink_brk = true;
	      break;
	    }
	  ...
	  if (l->l_info [VALIDX(DT_GNU_PRELINKED)] == NULL
		...
		_dl_debug_printf ("\n[vdprelink failed] %s: %s expect 0x%0x, found "
				  "0x%0x timestamp\n",
				  main_map->l_name[0] ? main_map->l_name : rtld_progname,
				  l->l_name, liblist->l_time_stamp,
				  l->l_info[VALIDX(DT_GNU_PRELINKED)] ?
				  l->l_info[VALIDX(DT_GNU_PRELINKED)]->d_un.d_val : 0);
	      prelink_brk = true;
	      break;
	    }
	...
	prelinked = true;
      else if (!prelink_brk && __builtin_expect (GLRO_dl_debug_mask & DL_DEBUG_LIBS, 0))
	_dl_debug_printf ("\n[vdprelink failed] %s: expect %d, found %d no. of loaded "
			  "libraries\n",
			  main_map->l_name[0] ? main_map->l_name : rtld_progname,
			  liblistend - t_liblist, r_listend - t_r_list);

      if (__builtin_expect (GLRO_dl_debug_mask & DL_DEBUG_LIBS, 0))
	_dl_debug_printf ("\nprelink checking: %s\n",
			  prelinked ? "ok" : "failed");

      if (!prelinked && __builtin_expect (GLRO_dl_debug_mask & DL_DEBUG_VDPRELINK, 0))
	{
	  _dl_debug_printf ("\n[vdprelink failed] %s: cannot continue, exiting\n",
			    main_map->l_name[0] ? main_map->l_name : rtld_progname);
	  _exit (EXIT_FAILURE);
	}
    }
    ...
    {
      if (__builtin_expect (GLRO_dl_debug_mask & DL_DEBUG_LIBS, 0))
	_dl_debug_printf ("\n[vdprelink failed] %s: not prelinked\n",
			  main_map->l_name[0] ? main_map->l_name : rtld_progname);
    }
...
#define LEN_AND_STR(str) sizeof (str) - 1, str
	...
	DL_DEBUG_STATISTICS },
      { LEN_AND_STR ("vdprelink"), "display vdprelink error and exit",
	DL_DEBUG_VDPRELINK },
