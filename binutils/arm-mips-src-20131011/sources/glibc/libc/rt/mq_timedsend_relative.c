...
#define __mq_send(a,b,c,d)      INLINE_SYSCALL(mq_timedsend,5,a,b,c,d,NULL)

/* Add message pointed by MSG_PTR to message queue MQDES, stop blocking
   on full message queue if ABS_TIMEOUT expires.  */
int
mq_timedsend_relative (mqd_t mqdes, const char *msg_ptr, size_t msg_len,
	      unsigned int msg_prio, const struct timespec *rel_timeout)
{
    ...
    vd_out_debug (&vd_relative_debug, REL_DBG_MQ_TIMEDSEND_RELATIVE, "mq_timedsend_relative");
    ...
    /* If the queue is in nonblocking mode just send and return */
    if ((old_attr.mq_flags & O_NONBLOCK) || (rel_timeout==NULL))
    {
        return __mq_send(mqdes, msg_ptr, msg_len, msg_prio);
    }
    ...
    /* Try to send message until success or timeout expired */
    while(1)
    {
        errno = 0;
        error = __mq_send (mqdes, msg_ptr, msg_len, msg_prio);
...
strong_alias (mq_timedsend_relative, __mq_timedsend_relative)
