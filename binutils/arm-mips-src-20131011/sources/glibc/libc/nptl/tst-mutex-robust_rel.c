...
do_test (void)
{
  ...
  pthread_mutexattr_setrobust_np(&a, PTHREAD_MUTEX_ROBUST_NP);
  ...
  ts.tv_nsec = 0;
  fprintf(stderr, "launching timedlock\n");
  err = pthread_mutex_timedlock_relative(&m, &ts);
  if (err == 0)
    {
    ...
    {
      fprintf (stderr,"timedlock error != ETIMEDOUT: %d\n", err);
      return 1;
    }
  else
    {
  ...
  (void) gettimeofday (&tv, NULL);/* needed for timersub later */

  ts.tv_sec = 2;	/* Wait 2 seconds.  */
  ts.tv_nsec = 0;
  if (pthread_mutex_timedlock_relative (&m, &ts) != 0)
    {
